...
it('renders 30 categories and keeps Enter navigation available', async () => {
  const categories = Array.from({ length: 30 }, (_, index) => ({ id: `category-${String(index + 1)}`, name: `Kategoria ${String(index + 1)}`, order: index }));
  mocked.value = { state: appState({ categories }), actions: appActions() };
...
it('locks the answering UI after the local deadline expires', async () => {
  mocked.value = { state: appState({ deadlineAt: Date.now() - 1 }), actions: appActions() };
  render(<AnsweringScreen />);
  expect(await screen.findByText('Czas minął')).toBeInTheDocument();
  expect(screen.getByText('Czekamy na prowadzącego…')).toBeInTheDocument();
  expect(screen.queryByRole('textbox')).not.toBeInTheDocument();
  expect(screen.queryByRole('button', { name: 'Gotowe' })).not.toBeInTheDocument();
});
...
it('shows sending and acknowledged states for host finalization', () => {
  const snapshot = {
    ...
    endMode: 'rounds', timeMode: 'per-answer-10s', settings: { answerDurationSeconds: 90, roundCount: 5, maxPlayers: 8, speedBonusEnabled: false },
  ...
  expect(screen.getByText('Kończymy rundę')).toBeInTheDocument();
  expect(screen.queryByRole('textbox')).not.toBeInTheDocument();
