...
it('renders authoritative final ranking with native guest actions', () => {
  mocked.value = createValue(); render(<ResultsScreen final />);
  expect(screen.getByText('20 pkt')).toBeInTheDocument();
  expect(screen.getByText('Klasyfikacja końcowa')).toBeInTheDocument();
  expect(screen.getByRole('button', { name: 'Jestem gotowy' })).toBeInTheDocument();
  expect(screen.getByRole('button', { name: 'Wyjdź z gry' })).toBeInTheDocument();
});
